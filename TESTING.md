...
|---|---|---|---|---|
| US01 — upload (compte) | `FileServiceTest` (token, hash mdp, extension bloquée, expiration hors bornes, taille) ; `DownloadTokensTest` ; front `upload.integ.spec` (états, > 1 Go, submit → `FileService`), `file.service.integ.spec` | `FileControllerIT` (Testcontainers PG + **MinIO**) : 201 + métadonnées persistées + objet dans le bucket ; 401 ; 400 `FORBIDDEN_FILE_TYPE` / `INVALID_EXPIRATION` / `VALIDATION` · `MinioStorageServiceIT` (store/retrieve/delete) | e2e upload→download (US02) | ☑ back + front (soumission câblée ; e2e attend US02) |
