...
| Image MinIO **`ghcr.io/denoyela-4913/silo`** (notre fork de `pgsty/silo`, construite en CI et publiée sur GHCR), **tag épinglé**, client `mcli` embarqué (plus d'image `mc` séparée) | `minio/minio` retiré de Docker Hub (10/2025), `quay.io/minio` privé (09/2026), `pgsty/minio` gelé depuis le renommage en `pgsty/silo` (08/2026) ; le fork garde le code même si l'amont disparaît ; jamais de `latest` pour rester reproductible ; correctifs de sécurité suivis dans les avis de SILO (dernier tag intégré : `RELEASE.2026-09-16`) | ☑ (`deploy/`, CI, Testcontainers) ; procédure de mise à jour dans [`MAINTENANCE.md`](MAINTENANCE.md) |
